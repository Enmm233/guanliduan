...
export default {
  ...
  methods: {
    ...
    submit: function() { 
      if(this.inputtext.typew==''){
        ...
        let mobile=this.inputtext.mobile.replace(/\s+/g, "");
        ...
        this.$post(APIUrl.login,{appid:APIUrl.appid,mobile:mobile,password:password,type:typew,timeStamp:APIUrl.timeStamp,sign:sign})
        .then((response) => {
            ...
            else(response.code==200)
            {
                // 本地存储
                var data = new Object();
                data.tokenw=response.data.token;
  ...
  created(){
    ...
    var that=this;
    var str1 = localStorage.getItem("tokenw");
                ...
                var mobileSelect1 = new MobileSelect({
                    ...
                    callback:function(indexArr, data){
                        if(data[0].id==3)
                        {
                                ...
                                $(".adminislo_iw").show();
                                that.$get(APIUrl.cart,{appid:APIUrl.appid,timeStamp:APIUrl.timeStamp,sign:sign})
                                .then((response) => {
                                        // 下拉菜单
