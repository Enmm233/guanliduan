<template>
  <article class="main">
      <article class="adminis_ti">
        <section class="adminis_tio">用户</section>
        <section class="adminisft_tir"><router-link :to="'/guanli_useraccdetails?id='+timeType">明细</router-link></section>
      </article>
      ...
      <article class="adminisft_yh">
        <ol>
          <li>
            <cite>总用户</cite>
            <aside>{{ adminList.member_num  }}</aside>
          </li>
          <li>
            <cite>活跃量</cite>
            <aside>{{ adminList.hy_num  }}</aside>
          </li>
          <li>
            <cite>沉滞量</cite>
            <aside>{{ adminList.cz_num  }}</aside>
          </li>
          <li>
            <cite>未审核</cite>
            <aside>{{ adminList.wsh_num  }}</aside>
          </li>
        </ol>
      </article>
...
export default {
    ...
    methods: {
      ...
      adminisua_px()
      {
            ...
            this.$store.commit('set_tripListe',timeType);
            ...
            this.$get(APIUrl.adminList,{appid:APIUrl.appid,timeStamp:APIUrl.timeStamp,timeType:timeType,sign:sign})
                .then((response) => {
                    ...
                    {
                        if(response.data.orderPrice.length!=0)
                        {
                            this.adminList=response.data;
                            this.echarts(response.data.orderPrice);
                            $(".placewr_ws").hide();
                        }
                        else
                        {
                        ...
                        this.adminList.member_num=response.data.member_num;
                        this.adminList.hy_num=response.data.hy_num;
                        this.adminList.cz_num=response.data.cz_num;
                        this.adminList.wsh_num=response.data.wsh_num;
                    }
                });
    ...
    mounted() {
      var that=this;
      var timeType=this.$store.state.tripliste;
      $(".adminisft_se address").removeClass("on");
      $(".adminisft_se address[data-id="+timeType+"]").addClass("on");
      // 实时监听屏幕宽度
      $(window).resize(function(){
          ...
          $("#adminisft_tb").removeAttr("style")
          setTimeout(() => {
            that.echarts(that.adminList.orderPrice);
          },350)
      });
    },
    destroyed(){
        var name=this.$route.name;
        if(name!='guanli_useraccdetails'){
            this.$store.commit('set_tripListe',1)
        }
    },
    created(){
