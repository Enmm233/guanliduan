<template>
  <article class="main">
    <article class="adminis_ti adminis_ti1">
      <section class="adminis_tio">供应单</section>
    </article>
    ...
    <article class="adminispo_list" data-page="1">
      <ol>
        <template v-for="(supplierList, index) in supplierList">
          <li>
            <section class="adminispo_list_ba">
              <router-link :to="'/gongying_purorderdetail?id='+supplierList.id">
                <section class="adminispo_list_d">
                  <cite>单号：{{ supplierList.order_sn }}</cite>
                  <template v-if="supplierList.status==0">
                    <dfn class="adminispo_list_ds2">采购中</dfn>
                  </template>
                  <template v-if="supplierList.status==1">
                    <dfn class="adminispo_list_ds1">已完成</dfn>
                  </template>
                  <template v-if="supplierList.status==2">
                    <dfn class="adminispo_list_ds3">已取消</dfn>
                  </template>
                </section>
                <section class="clear"></section>
                <section class="adminispo_list_s">
                  <aside>时间：{{ supplierList.date }}</aside>
                </section>
              </router-link>
            </section>
                  ...
                  if (response.code == 401) {
                    ...
                    if (response.data.length != 0) {
                      for (var i = 0; i < response.data.length; i++) {
                        that.supplierList.push(response.data[i]);
                      }
