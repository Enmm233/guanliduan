...
<article class="main">
  ...
  <article class="adminisdl_list" data-page="1">
    <ol>
      <template v-for="(vehicleOrderList, index) in vehicleOrderList">
        <li>
          <section class="adminisdl_list_ba">
              <section class="adminisdl_list_t">
                <cite>单号：{{  vehicleOrderList.order_sn }}</cite>
                <template v-if="vehicleOrderList.order_status==0">
                  <dfn class="adminisdl_list_ds3">待审核</dfn>
                </template>
                <template v-if="vehicleOrderList.order_status==1">
                  <dfn class="adminisdl_list_ds1">备货中</dfn>
                </template>
                 <template v-if="vehicleOrderList.order_status==2">
                  <dfn class="adminisdl_list_ds2">配送中</dfn>
                </template>
                <template v-if="vehicleOrderList.order_status==3">
                  <dfn class="adminisdl_list_ds4">已收货</dfn>
                </template>
                <template v-if="vehicleOrderList.order_status==4">
                  <dfn class="adminisdl_list_ds5">已取消</dfn>
                </template>
              </section>
              <section class="adminisdl_list_s">
                <router-link :to="'/shiji_orderdetail?id='+vehicleOrderList.id">
                  <address>收货人：{{  vehicleOrderList.contact }}</address>
                  <address>单位名称：{{  vehicleOrderList.nickname }}</address>
                  <address>配送日期：{{  vehicleOrderList.send_time }}</address>
                </router-link>
...
export default {
  ...
  methods: {
    ...
    submit: function() { 
          ...
          this.$get(APIUrl.vehicleOrderList,{appid:APIUrl.appid,send_time:send_time,keyword:keyword,timeStamp:APIUrl.timeStamp,page:1,sign:sign})
          .then((response) => {
              ...
              else if(response.code==200)
              {
                  if(response.data.length!=0)
                  {
                      $(".placewr_ws").hide();
                      $(".vueappsy_mode").show();
                      this.vehicleOrderList=response.data;
    ...
    scrolle() {
      ...
      $(window).off("scroll").on('scroll',function(){   
        ...
        if(parseInt(nScrollTop) + parseInt(nScrollHight) >= nDivHight-3)
          {
                if(that.$route.name=='shiji_order')
                {
                    ...
                    that.$get(APIUrl.vehicleOrderList,{appid:APIUrl.appid,page:(page+1),send_time:send_time,keyword:keyword,timeStamp:APIUrl.timeStamp,sign:sign1})
                    .then((response) => {
                      ...
                      else if(response.code==200)
                      {
                          if(response.data.length!=0)
                          {
                              ...
                              {
                                  that.vehicleOrderList.push(response.data[i]);
                              }
  ...
  created(){
      ...
      // 订单列表
      let sign=this.$md5('appid='+APIUrl.appid+'&timeStamp='+APIUrl.timeStamp+APIUrl.appsecret);
      this.$get(APIUrl.vehicleOrderList,{appid:APIUrl.appid,timeStamp:APIUrl.timeStamp,page:1,sign:sign})
        .then((response) => {
            this.$Indicator.close();
            if(response.code==401)
            {
