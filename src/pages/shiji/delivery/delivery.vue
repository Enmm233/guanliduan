...
<article class="main">
  ...
  <article class="adminisdl_list adminiszd_list">
    <ol>
      <template v-for="(vehicleList, index) in vehicleList">
        <li>
          <section class="adminisdl_list_ba">
              <section class="adminisdl_list_t">
                <cite>单号：{{ vehicleList.order_sn }}</cite>
                <template v-if="vehicleList.order_status==0">
                  <dfn class="adminisdl_list_ds3">待审核</dfn>
                </template>
                <template v-if="vehicleList.order_status==1">
                  <dfn class="adminisdl_list_ds1">备货中</dfn>
                </template>
                 <template v-if="vehicleList.order_status==2">
                  <dfn class="adminisdl_list_ds2">配送中</dfn>
                </template>
                <template v-if="vehicleList.order_status==3">
                  <dfn class="adminisdl_list_ds4">已收货</dfn>
                </template>
                <template v-if="vehicleList.order_status==4">
                  <dfn class="adminisdl_list_ds5">已取消</dfn>
                </template>

              </section>
              <section class="adminisdl_list_s">
                <a :href="'tel:'+vehicleList.mobile" class="adminisdl_list_sa"><i class="fa fa-phone-square"></i></a>
                <router-link :to="'/shiji_deliverydetail?id='+vehicleList.id">
                  <address>收货人：{{ vehicleList.contact }}</address>
                  <address>收货手机：{{ vehicleList.mobile }}</address>
                  <address>单位名称：{{ vehicleList.nickname }}</address>
                  <address>收货地址：{{ vehicleList.address }}</address>
                </router-link>
...
export default {
  ...
  created(){
      ...
      this.$get(APIUrl.vehicleList,{appid:APIUrl.appid,timeStamp:APIUrl.timeStamp,sign:sign})
        .then((response) => {
